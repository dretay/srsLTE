...
static char *rf_args = "";
static char *rf_dev = "";
static float rf_amp = 0.8f, rf_gain = 60.0, rf_freq = 2400000000;
...
static int sf_n_re, sf_n_samples;
...
void base_init() {
  ...
  for (i = 0; i < SRSLTE_MAX_PORTS; i++) {
    sf_buffer[i] = srslte_vec_malloc(sizeof(cf_t) * (uint32_t)sf_n_re);
    if (!sf_buffer[i]) {
      perror("malloc");
      exit(-1);
    }
  ...
  for (i = 0; i < SRSLTE_MAX_PORTS; i++) {
    output_buffer[i] = srslte_vec_malloc(sizeof(cf_t) * (uint32_t)sf_n_samples);
    if (!output_buffer[i]) {
      perror("malloc");
      exit(-1);
    }
  ...
  while ((nf < nof_frames || nof_frames == -1) && !go_exit) {
    for (sf_idx = 0; sf_idx < SRSLTE_NOF_SF_X_FRAME && (nf < nof_frames || nof_frames == -1); sf_idx++) {
      ...
      if (mch_table[sf_idx] == 1 && mbsfn_area_id > -1) {
        ...
        dl_sf.tti = (uint32_t)(nf * 10 + sf_idx);
        for (i = 0; i < cell.nof_ports; i++) {
